...
const formState = reactive({ factoryId: '', factoryName: '' });
...
async function onSubmit() {
  ...
  const factoryId = String(formState.factoryId || '').trim();
  const name = formState.factoryName.trim();
  if (!factoryId || !name) {
    masterDataErrorMessage.value = 'Factory Id and Factory Name are required.';
    return;
  }
  ...
  try {
    const res = await saveFactory(payload, formMode.value === 'edit');
    if (res?.success) {
      showToast(formMode.value === 'edit' ? 'Factory updated' : 'Factory created');
      resetForm();
      closeModal();
    } else {
...
function editItem(item) {
  formMode.value = 'edit';
  formState.factoryId = String(item.factoryId || '');
  formState.factoryName = item.factoryName || '';
  isModalOpen.value = true;
}
...
async function deleteItem(item) {
  ...
  try {
    const res = await removeFactory(String(item.factoryId || ''));
    if (res?.success) {
...
const filteredFactoryItems = computed(() => {
  const keyword = (searchText.value || '').toLowerCase();
  if (!keyword) return factoryItems.value;
  return factoryItems.value.filter((item) =>
    String(item.factoryId || '').toLowerCase().includes(keyword) ||
...
const totalItems = computed(() => filteredFactoryItems.value.length);
const paginatedFactoryItems = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value;
  const end = start + pageSize.value;
  return filteredFactoryItems.value.slice(start, end);
});
watch([searchText, factoryItems], () => {
  currentPage.value = 1;
});
</script>
...
<template>
  <SettingsCrudLayout
    ...
    search-placeholder="Search factory..."
    total-label=""
    showing-label=""
  >
    <template #actions>
      ...
      <button class="bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 ui-btn px-4 py-2 rounded-lg transition active:scale-95" @click="loadFactoriesCrud">
        Refresh
      </button>
    </template>
        ...
        <table class="w-full text-left border-collapse ui-table">
          <thead>
            <tr class="border-b border-slate-200 font-bold">
              <th class="p-4">Factory Id</th>
              <th class="p-4">Factory Name</th>
              <th class="p-4">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            <tr v-if="paginatedFactoryItems.length === 0">
              <td colspan="3" class="p-8 text-center text-slate-400 font-semibold">No factories found.</td>
            </tr>
            <tr v-else v-for="item in paginatedFactoryItems" :key="item.factoryId">
              <td class="p-4 text-slate-700 font-semibold">{{ item.factoryId }}</td>
              <td class="p-4"><strong class="text-slate-800">{{ item.factoryName }}</strong></td>
              <td class="p-4">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/45 p-4">
      <div class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <h3 class="m-0 text-base font-extrabold text-slate-800">{{ formMode === 'create' ? 'Add Factory' : 'Edit Factory' }}</h3>
          <ModalCloseButton @click="closeModal" />
        </div>
        <div class="space-y-3 p-5">
          <div class="flex flex-col gap-1 text-left">
            <label class="text-xs font-bold text-slate-500">Factory Id</label>
            <input
              type="text"
              v-model="formState.factoryId"
          ...
          <div class="flex flex-col gap-1 text-left">
            <label class="text-xs font-bold text-slate-500">Factory Name</label>
            <input
              type="text"
              v-model="formState.factoryName"
