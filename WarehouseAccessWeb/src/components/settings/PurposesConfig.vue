...
const formState = reactive({ purposeId: '', purposeName: '' });
...
async function onSubmit() {
  ...
  const payload = {
    purposeId: formState.purposeId ? String(formState.purposeId) : null,
    purposeName: name
  };
...
function editItem(item) {
  formMode.value = 'edit';
  formState.purposeId = String(item.purposeId || '');
...
async function deleteItem(item) {
  ...
  try {
    const res = await removePurpose(String(item.purposeId || ''));
    if (res?.success) {
...
const filteredPurposeItems = computed(() => {
  const keyword = (searchText.value || '').toLowerCase();
  if (!keyword) return purposeItems.value;
  return purposeItems.value.filter((item) =>
    String(item.purposeId || '').toLowerCase().includes(keyword) ||
...
const totalItems = computed(() => filteredPurposeItems.value.length);
const paginatedItems = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value;
  return filteredPurposeItems.value.slice(start, start + pageSize.value);
});
watch([searchText, purposeItems], () => {
  currentPage.value = 1;
});
</script>
...
<template>
  ...
  >
    <template #actions>
      ...
      <button class="bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 ui-btn px-4 py-2 rounded-lg transition active:scale-95" @click="loadPurposesCrud">
        Refresh
      </button>
    </template>
        ...
        <table class="w-full text-left border-collapse ui-table">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-if="paginatedItems.length === 0">
              <td colspan="3" class="p-8 text-center text-slate-400 font-semibold">No purposes found.</td>
            </tr>
            <tr v-else v-for="item in paginatedItems" :key="item.purposeId">
              <td class="p-4 text-slate-700 font-semibold">{{ item.purposeId }}</td>
              <td class="p-4"><strong class="text-slate-800">{{ item.purposeName }}</strong></td>
              <td class="p-4">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/45 p-4">
      <div class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <h3 class="m-0 text-base font-extrabold text-slate-800">{{ formMode === 'create' ? 'Add Purpose' : 'Edit Purpose' }}</h3>
